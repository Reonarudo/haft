...
namespace {
    ...
    std::set<BasicBlock*> notreallyvisited;
    ...
    class InvokeBBMap{
        typedef std::map<Value*, IRBuilder<>> InvokeBBMapType;
        InvokeBBMapType ivkBBM;
        
    public:
        InvokeBBMap(): ivkBBM() {}
        
        void add(Value* v, IRBuilder<>& b) {
            bool isnew;
            InvokeBBMapType::iterator it;
            
            std::tie(it, isnew) = ivkBBM.insert(std::make_pair(v, b));
            assert (isnew && "value already has a shadow");
        }
        
        IRBuilder<> getBBBuilder(Value *v, Value *inst_debug) {
            InvokeBBMapType::iterator it = ivkBBM.find(v);
            if (it == ivkBBM.end()){
                errs() << "Value '" << *v << "' has no shadow (for Instr '" << *inst_debug << "')\n";
            }
            assert(it != ivkBBM.end() && "value has no shadow");
            
            return it->second;
        }
        
        bool hasShadow(Value *v) {
            return ivkBBM.end() != ivkBBM.find(v);
        }
    ...
    public:
        ...
        //        void addInvokeShadows(Instruction* I){
        //            
        //            IRBuilder<> b=invokeShadows.getBBBuilder(<#llvm::Value *v#>, <#llvm::Value *inst_debug#>)
        //            
        ...
        void checkInst(Instruction* I) {
            ...
            switch (I->getOpcode()) {
                ...
                case Instruction::Invoke:
                {
                    ...
                    //errs() << "add check instruction(s) before "<< *I <<"\n";
                    IRBuilder<> irBuilder(instIt->getParent(), instIt);
                    //errs() << "1\n";
                    checkInstOperands(I, irBuilder);
                    //errs() << "2\n";
    ...
    public:
        ...
        virtual bool runOnFunction(Function &F) {
            ...
            //errs() << "111111111111111111111111111111111111111111111111111111111\n";
            ...
            for (df_iterator<DomTreeNode*> DI = df_begin(DT.getRootNode()),	E = df_end(DT.getRootNode()); DI != E; ++DI){
                ...
                for (BasicBlock::iterator instIt = BB->begin (); instIt != BB->end (); ) {
                    ...
                    //errs() << "^£££££££\n";
                    if (dontcheck.count(&*instIt)!=0) {
                        errs() << "Not checking: " <<*instIt << "\n";
                        instIt = nextIt;
                        //continue;
                    }
            ...
            //errs() << "C\n";
            //errs() << "444444444444444444444444444444444444444444444444444444444\n";
